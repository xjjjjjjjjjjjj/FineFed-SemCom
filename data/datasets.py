...
NUM_DATASET_WORKERS = 0 #进程数 用的windows所以是0
SCALE_MIN = 0.75
SCALE_MAX = 0.95
...
class Datasets(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.im_height == 32 and self.im_width == 32:  #CIFAR10 的测试
...
class Datasets1(Dataset): #自定义的一个 训练CIFAR10的
    def __init__(self, data_dir):
        self.data_dir = data_dir
        self.imgs = []
        for dir in self.data_dir:
...
def get_loader(args, config):
    if args.trainset == 'DIV2K':
        ...
        train_dataset = Datasets1(config.train_data_dir)
